 ...
 *
 * <p>One case per Parquet primitive kind. Fixed-width primitive variants hold a read-only primitive buffer
 * ({@link IntBuffer}, {@link LongBuffer}, etc.) so dictionary lookups are unboxed and the dictionary owns a single
 * contiguous heap allocation. The boolean case uses a {@link BitSet}. Binary cases (BYTE_ARRAY, FIXED_LEN_BYTE_ARRAY,
 * INT96) hold a {@code List} of owned, read-only {@link MemorySegment}s so repeat lookups never share a mutable cursor.
 */
