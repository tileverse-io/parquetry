...
public final class NestedVectorAssembler {
    ...
    private static ColumnPath findFirstDescendantLeafPath(
            Field.Group group, List<String> groupPath, Map<ColumnPath, ColumnVector> leafVectors) {
        for (Field child : group.children()) {
            ColumnPath childPath = new ColumnPath(concatPath(groupPath, child.name()));
            if (child instanceof Field.Primitive) {
            ...
            ColumnPath nested =
                    findFirstDescendantLeafPath((Field.Group) child, concatPath(groupPath, child.name()), leafVectors);
            if (nested != null) {
                return nested;
            }
        }
    ...
    private static void markDescendantLeavesHidden(
        ...
        for (Field child : group.children()) {
            ...
            markDescendantLeavesHidden((Field.Group) child, childPath, leafVectors, hiddenLeaves);
        }
    }
    ...
    private static MapKeyValuePaths findMapKeyValuePaths(
            Field.Group mapGroup, List<String> mapGroupPath, Map<ColumnPath, ColumnVector> leafVectors) {
        if (mapGroup.children().isEmpty()) {
            return null;
        }
        Field middleChild = mapGroup.children().get(0);
        if (!(middleChild instanceof Field.Group middle) || middle.children().size() < 2) {
            return null;
        }
        List<String> middlePath = concatPath(mapGroupPath, middle.name());
        Field keyField = middle.children().get(0);
        Field valueField = middle.children().get(1);
    ...
    private static ColumnPath leafPathOrFirstDescendant(
            Field field, List<String> path, Map<ColumnPath, ColumnVector> leafVectors) {
        if (field instanceof Field.Primitive) {
            ColumnPath p = new ColumnPath(path);
            return leafVectors.containsKey(p) ? p : null;
        }
        return findFirstDescendantLeafPath((Field.Group) field, path, leafVectors);
    }
    ...
    /**
     * Classifies a {@link Field.Group} as LIST, MAP, or STRUCT.
     *
     ...
     */
    private static GroupKind classify(Field.Group group) {
