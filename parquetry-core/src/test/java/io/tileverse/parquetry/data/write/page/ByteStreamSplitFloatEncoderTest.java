...
class ByteStreamSplitFloatEncoderTest {
    ...
    void encodingMarkerIsByteStreamSplit() {
        assertThat(new ByteStreamSplitFloatEncoder().parquetEncoding()).isEqualTo(Encoding.BYTE_STREAM_SPLIT);
    }
    ...
    static Stream<Arguments> roundTripCases() {
        return Stream.of(
                ...
                Arguments.of("everyday", new float[] {1.0f, -1.0f, 3.14f, -2.71f, 1e30f, -1e30f, 0.0f, 0.5f}),
                Arguments.of("allSame", new float[] {0.5f, 0.5f, 0.5f, 0.5f}));
    }
