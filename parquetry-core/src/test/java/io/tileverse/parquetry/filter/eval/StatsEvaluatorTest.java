...
class StatsEvaluatorTest {
    ...
    void eqValueAboveMaxIsEliminated() {
        ColumnStatsLookup cols = single("year", PrimitiveKind.INT32, intStats(2010, 2020, 0));
    ...
    void eqSingleDistinctValueWithoutNullsPassesAll() {
        ColumnStatsLookup cols = single("year", PrimitiveKind.INT32, intStats(2020, 2020, 0));
    ...
    void ltEqAtMinPassesAll() {
        ColumnStatsLookup cols = single("year", PrimitiveKind.INT32, intStats(2010, 2010, 0));
    ...
    void isNullWithAllNullsPassesAll() {
        ColumnStatsLookup cols = single("year", PrimitiveKind.INT32, intStats(2010, 2020, ROW_COUNT));
        PruningDecision d = StatsEvaluator.evaluate(col("year").isNull(), cols, ROW_COUNT);
        assertThat(d).isInstanceOf(PruningDecision.PassedAll.class);
    }
    ...
    void missingMinMaxYieldsNotApplied() {
        Statistics noBounds = new Statistics(
                ...
                Optional.empty(),
                Optional.empty());
        ColumnStatsLookup cols = single("year", PrimitiveKind.INT32, noBounds);
        PruningDecision d = StatsEvaluator.evaluate(col("year").eq(2020), cols, ROW_COUNT);
        assertThat(d).isInstanceOf(PruningDecision.NotApplied.class);
    }
    ...
    void doubleColumnLtPrunes() {
        ColumnStatsLookup cols = single("price", PrimitiveKind.DOUBLE, doubleStats(1.0, 5.0, 0));
    ...
    void stringColumnEqInRange() {
        ColumnStatsLookup cols = single("name", PrimitiveKind.BYTE_ARRAY, binaryStats("alpha", "omega", 0));
    ...
    void stringColumnEqBelowRangeIsEliminated() {
        ColumnStatsLookup cols = single("name", PrimitiveKind.BYTE_ARRAY, binaryStats("delta", "omega", 0));
        PruningDecision d = StatsEvaluator.evaluate(col("name").eq("alpha"), cols, ROW_COUNT);
        assertThat(d).isInstanceOf(PruningDecision.Eliminated.class);
    }

    private static ColumnStatsLookup single(String name, PrimitiveKind kind, Statistics stats) {
        Map<ColumnPath, ColumnStats> map = new HashMap<>();
        map.put(ColumnPath.of(name), new ColumnStats(kind, stats));
        return path -> Optional.ofNullable(map.get(path));
    }

    private static ColumnStatsLookup empty() {
