...
class ColumnIndexEvaluatorTest {
    ...
    void eqInMiddlePageNarrows() {
        ColumnPageStatsLookup cols = year3Pages();
    ...
    void missingIndexYieldsNotApplied() {
        ColumnPageStatsLookup cols = empty();
        PruningDecision d = ColumnIndexEvaluator.evaluate(col("year").eq(2020), cols, ROW_GROUP_ROWS);
    ...
    void allNullPageDoesNotMatchValueComparison() {
        ColumnPageStatsLookup cols = singleColumn(
                "year",
                PrimitiveKind.INT32,
    ...
    private static ColumnPageStatsLookup year3Pages() {
        return singleColumn(
                "year",
                PrimitiveKind.INT32,
    ...
    private static ColumnPageStatsLookup singleColumn(
            String name,
            PrimitiveKind kind,
        ...
        Map<ColumnPath, ColumnPageStats> map = new HashMap<>();
        map.put(ColumnPath.of(name), new ColumnPageStats(kind, idx, off));
        return path -> Optional.ofNullable(map.get(path));
    }

    private static ColumnPageStatsLookup empty() {
