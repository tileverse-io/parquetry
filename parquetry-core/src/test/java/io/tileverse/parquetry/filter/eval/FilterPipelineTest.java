...
class FilterPipelineTest {
    ...
    private static FilterPipeline.RowGroupInputs inputs(
            long rows, ColumnStatsLookup stats, DictionaryLookup dicts, ColumnPageStatsLookup pageIndexes) {
        return new FilterPipeline.RowGroupInputs(rows, stats, dicts, pageIndexes);
    }

    private static ColumnStatsLookup statsOf(String name, PrimitiveKind kind, Statistics stats) {
    ...
    private static DictionaryLookup noDicts() {
        return path -> Optional.empty();
    }

    private static ColumnPageStatsLookup noPageIndex() {
        return path -> Optional.empty();
    }
