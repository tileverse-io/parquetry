...
import io.tileverse.parquetry.read.FetchedColumnChunk;
import io.tileverse.parquetry.read.LevelMaximaResolver;
 ...
 */
public final class RealColumnFetcher implements ColumnFetcher {
    ...
    private static PageHeader readPageHeader(ByteBuffer buffer, ColumnPath path) {
        try {
            ...
            return Format.readPageHeader(stream);
        } catch (IOException e) {
            throw new ParquetFormatException(
