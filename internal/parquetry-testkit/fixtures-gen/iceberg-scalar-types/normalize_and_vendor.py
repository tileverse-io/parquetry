"""Normalize the raw Iceberg scalar-types warehouse and vendor it as a fixture.

Run on the host (needs fastavro; system python3 has it) AFTER scalar-types-gen
has produced ./scalar-types-gen/work/warehouse/scalars.

Makes the table portable, mirroring iceberg-deletes/normalize_and_vendor_equality.py:
  - rewrites the absolute generation path to a clean logical root
    (file:///iceberg-scalar-types/scalars) in the metadata json and the manifest
    list / manifest Avro (every spelling emitted by the Iceberg Java writer: the
    schemed file: form in the metadata and the bare filesystem path in the
...
The Parquet files are left byte-for-byte untouched.
"""
...
SOURCE = HERE / "scalar-types-gen" / "work" / "warehouse" / "scalars"
DEST = HERE.parents[1] / "src" / "main" / "resources" / "iceberg-scalar-types" / "scalars"
CLEAN_ROOT = "file:///iceberg-scalar-types/scalars"
