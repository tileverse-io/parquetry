...
/**
 * Reads the normalized, vendored fixture back through Iceberg's own reader to prove it still
 * resolves and yields the same live-row set after the path rewrite.
 *
 * <p>The argument is a directory whose internal paths already point to the table location (the
 * caller stages the vendored tree under that location). The verifier opens the table from its
 * metadata and asserts the live ids match the oracle.
 */
