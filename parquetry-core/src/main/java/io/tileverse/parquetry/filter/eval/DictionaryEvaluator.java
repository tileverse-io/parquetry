 ...
 * And/Or flattened).
 */
public final class DictionaryEvaluator {
    ...
    public static PruningDecision evaluate(
            io.tileverse.parquetry.filter.Predicate predicate, DictionaryLookup dictionaries) {
        return switch (predicate) {
            ...
            case io.tileverse.parquetry.filter.Predicate.BboxIntersects _ ->
                new PruningDecision.NotApplied(TIER, "BboxIntersects not handled at DICTIONARY tier");
        };
    }

    private static PruningDecision evalAnd(
            List<io.tileverse.parquetry.filter.Predicate> children, DictionaryLookup dicts) {
    ...
    private static PruningDecision evalNotLeaf(io.tileverse.parquetry.filter.Predicate child, DictionaryLookup dicts) {
    ...
    private static PruningDecision evalLeaf(
            ColumnPath col, DictionaryLookup dicts, Predicate<Object> matches, String opLabel) {
