"""Normalize the raw Iceberg fresh row-lineage warehouse and vendor it as a fixture.

Run on the host (needs pyarrow + fastavro; reuse the iceberg-deletes venv) AFTER
fresh-gen has produced ./fresh-gen/work/warehouse/events.
...
Makes the table portable, mirroring the iceberg-deletes normalize scripts:
  - rewrites the absolute generation path to a clean logical root
    (file:///iceberg-row-lineage/fresh) in the metadata json and the manifest
...
to them. The row-lineage bases (metadata next-row-id, per-snapshot first-row-id,
and the manifest-list ManifestFile.first_row_id) are numeric and path-free; the
rewrite leaves them untouched.
"""
...
SOURCE = HERE / "fresh-gen" / "work" / "warehouse" / "events"
DEST = HERE.parents[1] / "src" / "main" / "resources" / "iceberg-row-lineage" / "fresh"
CLEAN_ROOT = "file:///iceberg-row-lineage/fresh"
