...
public final class StatsEvaluator {
     ...
     */
    public static PruningDecision evaluate(Predicate predicate, ColumnStatsLookup columns, long rowCount) {
    ...
    private static PruningDecision evaluateAnd(List<Predicate> children, ColumnStatsLookup cols, long rowCount) {
    ...
    private static PruningDecision evaluateOr(List<Predicate> children, ColumnStatsLookup cols, long rowCount) {
     ...
     */
    private static PruningDecision evaluateNotLeaf(Predicate child, ColumnStatsLookup cols, long rowCount) {
    ...
    private static PruningDecision evalEq(ColumnPath col, Value v, ColumnStatsLookup cols, long rowCount) {
    ...
    private static PruningDecision evalNotEq(ColumnPath col, Value v, ColumnStatsLookup cols, long rowCount) {
        return withRange(col, cols, range -> {
            int cmpMin = compare(range.kind(), v, range.min());
            int cmpMax = compare(range.kind(), v, range.max());
            if (cmpMin < 0 || cmpMax > 0) {
    ...
    private static PruningDecision evalLt(ColumnPath col, Value v, ColumnStatsLookup cols) {
        return withRange(col, cols, range -> {
    ...
    private static PruningDecision evalLtEq(ColumnPath col, Value v, ColumnStatsLookup cols) {
        return withRange(col, cols, range -> {
            if (compare(range.kind(), v, range.min()) < 0) {
    ...
    private static PruningDecision evalGt(ColumnPath col, Value v, ColumnStatsLookup cols) {
        return withRange(col, cols, range -> {
            if (compare(range.kind(), v, range.max()) >= 0) {
    ...
    private static PruningDecision evalGtEq(ColumnPath col, Value v, ColumnStatsLookup cols) {
        return withRange(col, cols, range -> {
            if (compare(range.kind(), v, range.max()) > 0) {
    ...
    private static PruningDecision evalIn(ColumnPath col, List<Value> values, ColumnStatsLookup cols) {
        return withRange(col, cols, range -> {
            for (Value v : values) {
                int cmpMin = compare(range.kind(), v, range.min());
                int cmpMax = compare(range.kind(), v, range.max());
    ...
    private static PruningDecision evalIsNull(ColumnPath col, ColumnStatsLookup cols, long rowCount) {
    ...
    private static PruningDecision evalIsNotNull(ColumnPath col, ColumnStatsLookup cols, long rowCount) {
        Optional<Long> nullCountOpt = lookupNullCount(col, cols);
        if (nullCountOpt.isEmpty()) {
    ...
    private static Optional<Long> lookupNullCount(ColumnPath col, ColumnStatsLookup cols) {
    ...
    private static PruningDecision withRange(
            ColumnPath col, ColumnStatsLookup cols, java.util.function.Function<DecodedRange, PruningDecision> f) {
        Optional<ColumnStats> stats = cols.get(col);
        if (stats.isEmpty()) {
            return new PruningDecision.NotApplied(TIER, "no stats for " + col.dot());
        }
        ColumnStats cs = stats.get();
