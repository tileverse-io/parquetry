 ...
 */
public final class ColumnIndexEvaluator {
     ...
     */
    public static PruningDecision evaluate(Predicate predicate, ColumnPageStatsLookup columns, long rowGroupRowCount) {
    ...
    private static Optional<RowRanges> matchRanges(
            Predicate predicate, ColumnPageStatsLookup columns, long rowGroupRowCount) {
    ...
    private static Optional<RowRanges> combineAnd(
            List<Predicate> children, ColumnPageStatsLookup cols, long rowGroupRowCount) {
    ...
    private static Optional<RowRanges> leafRanges(
            ColumnPath col, ColumnPageStatsLookup cols, long rowGroupRowCount, PageMatcher matcher) {
        Optional<ColumnPageStats> opt = cols.get(col);
        if (opt.isEmpty()) {
            return Optional.empty();
        }
        ColumnPageStats stats = opt.get();
    ...
    private static Optional<RowRanges> nullLeafRanges(
            ColumnPath col, ColumnPageStatsLookup cols, long rowGroupRowCount, boolean wantNulls) {
        Optional<ColumnPageStats> opt = cols.get(col);
        if (opt.isEmpty()) {
            return Optional.empty();
        }
    ...
    private static int compare(Value a, Value b) {
        return switch (a) {
            ...
            when b instanceof Value.BinaryVal(ByteBuffer bv) ->
                compareBytes(ByteBuffer.wrap(av.getBytes(StandardCharsets.UTF_8)), bv);
            case Value.BinaryVal(ByteBuffer av)
            when b instanceof Value.BinaryVal(ByteBuffer bv) -> compareBytes(av, bv);
