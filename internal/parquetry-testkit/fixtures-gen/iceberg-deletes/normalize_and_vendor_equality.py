"""Normalize the raw Iceberg equality-delete warehouse and vendor it as a fixture.

Run on the host (needs pyarrow + fastavro; see README.md's venv note) AFTER
equality-gen has produced ./equality-gen/work/warehouse/events.

Makes the table portable, mirroring normalize_and_vendor.py (the positional
fixture) and iceberg-partitioned/generate_fixture.py:
  - rewrites the absolute generation path to a clean logical root
    (file:///iceberg-deletes/equality) in the metadata json and the manifest
...
fields category and flag); no Parquet path rewrite applies to it.
"""
...
SOURCE = HERE / "equality-gen" / "work" / "warehouse" / "events"
DEST = HERE.parents[1] / "src" / "main" / "resources" / "iceberg-deletes" / "equality"
CLEAN_ROOT = "file:///iceberg-deletes/equality"
