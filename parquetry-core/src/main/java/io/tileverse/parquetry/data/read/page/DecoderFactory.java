 ...
 * mandatory; an absent dictionary produces a {@link MalformedFileException}.
 *
...
public final class DecoderFactory {
     ...
     * @throws MalformedFileException for invalid (encoding, kind) combinations or missing required arguments
        ...
        return switch (kind) {
            ...
            case FIXED_LEN_BYTE_ARRAY ->
                new PlainFixedLenBinaryDecoder(typeLength.orElseThrow(
                        () -> new MalformedFileException("typeLength required for PLAIN FIXED_LEN_BYTE_ARRAY")));
        };
    }

    private static PageDecoder<?> rleDecoder(PrimitiveKind kind) {
        if (kind != PrimitiveKind.BOOLEAN) {
            throw new MalformedFileException("RLE data-page encoding is only valid for BOOLEAN; got " + kind);
        }
    ...
    private static PageDecoder<?> dictionaryDecoder(Optional<Dictionary<?>> dictionary, PrimitiveKind kind) {
        Dictionary<?> dict = dictionary.orElseThrow(() -> new MalformedFileException(
    ...
    private static PageDecoder<?> deltaBinaryPackedDecoder(PrimitiveKind kind) {
        return switch (kind) {
            ...
            default ->
                throw new MalformedFileException("DELTA_BINARY_PACKED is only valid for INT32/INT64; got " + kind);
        };
    }

    private static PageDecoder<?> deltaLengthByteArrayDecoder(PrimitiveKind kind) {
        if (kind != PrimitiveKind.BYTE_ARRAY) {
            throw new MalformedFileException("DELTA_LENGTH_BYTE_ARRAY is only valid for BYTE_ARRAY; got " + kind);
        }
    ...
    private static PageDecoder<?> deltaByteArrayDecoder(PrimitiveKind kind) {
        if (kind != PrimitiveKind.BYTE_ARRAY) {
            throw new MalformedFileException("DELTA_BYTE_ARRAY is only valid for BYTE_ARRAY; got " + kind);
        }
